...
export default function AdminReportDetail() {
  ...
  function getAllowedTransitions(current) {
    switch (current) {
      case "submitted":
        return ["acknowledged", "investigating", "resolved", "closed"];
      case "acknowledged":
  ...
  useEffect(() => {
    ...
    fetch(
      (process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") +
        `/events/slug/${eventSlug}/users?role=Responder&limit=1000`,
      { credentials: "include" },
    )
      ...
      .then((data) => {
        fetch(
          (process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000") +
            `/events/slug/${eventSlug}/users?role=Admin&limit=1000`,
          { credentials: "include" },
        )
  ...
  const handleUpdateMeta = async (e) => {
    ...
    if (
      (selectedState === "resolved" || selectedState === "closed") &&
      !resolution.trim()
    ) {
      setAssignmentError(
        "Resolution is required when report is resolved or closed.",
      );
  ...
  const handleStateChange = async (e) => {
    ...
    if (
      ...
      setStateChangeError(
        "Resolution is required when resolving or closing a report.",
      );
  ...
  if (loading)
    return (
      <Card className="max-w-3xl mx-auto p-4 sm:p-8 mt-8">Loading...</Card>
    );
  if (error)
    return (
      <Card className="max-w-3xl mx-auto p-4 sm:p-8 mt-8 text-red-500 dark:text-red-400">
        {error}
      </Card>
    );
      ...
      <form onSubmit={handleUpdateMeta} className="mb-6">
            ...
            <tr>
              ...
              <td>
                {isResponderOrAbove ? (
                  ...
                  >
                    <option value={report.state}>
                      {report.state.charAt(0).toUpperCase() +
                        report.state.slice(1)}
                    </option>
        ...
        <div className="flex gap-4 mt-4 items-center">
          ...
          >
            {assignmentLoading
              ? "Saving..."
              : "Save Assignment/Severity/Resolution"}
          </Button>
          {assignmentError && (
                    ...
                    <div className="space-y-2">
                      ...
                      {isResponderOrAbove && (
                        <div>
                          ...
                          >
                            <option value="public">
                              Public (visible to all involved)
                            </option>
                            <option value="internal">
                              Internal (responders/admins only)
                            </option>
                          </select>
                        </div>
                      )}

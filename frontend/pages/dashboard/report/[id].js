...
export default function ReportDetail({ initialReport, error }) {
  ...
  // Fetch user info and roles for this event
  useEffect(() => {
    ...
    if (eventId) {
      console.log('Fetching user roles from:', `/api/events/${eventId}/users`);
      fetch(`/api/events/${eventId}/users`, { credentials: 'include' })
        .then(res => res.ok ? res.json() : null)
        .then(data => {
  ...
  }, [router.query.eventId, user && user.id]);
  ...
  const handleStateChange = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/events/${report.eventId}/reports/${report.id}/state`, {
        method: 'PATCH',
...
export async function getServerSideProps(context) {
  ...
  let error = null;
  if (!id || !eventId) {
    return { props: { initialReport: null, error: 'Missing report ID or event ID.' } };
  }
  try {
    const fetchUrl = `${process.env.BACKEND_API_URL}/events/${eventId}/reports/${id}`;
    console.log('Fetching report detail from:', fetchUrl);

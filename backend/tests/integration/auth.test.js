...
describe('Auth endpoints', () => {
  ...
  describe('POST /register', () => {
    ...
    it('should fail if password is weak', async () => {
      const res = await request(app).post('/register').send({ email: 'weak@example.com', password: 'password', name: 'Weak User' });
      expect(res.statusCode).toBe(400);
    ...
    it('should fail if email already registered', async () => {
      await request(app).post('/register').send({ email: 'dupe@example.com', password: 'StrongPass123!', name: 'Dupe' });
      const res = await request(app).post('/register').send({ email: 'dupe@example.com', password: 'StrongPass123!', name: 'Dupe' });
